...
class GetStakingRatesTool extends Tool {
  ...
  async fetchThalaRate() {
    try {
      ...
      const stakingAPR = ((rewards / staked) * 4380 * 100).toFixed(2) + "%";

      return {
        staking: { apr: parseFloat(stakingAPR) || 7.5, product: "sthAPT", totalStaked, features: ["Boosted yields", "Governance"] },
...
const agent = createReactAgent({
  ...
  messageModifier: `
    ...
    Capabilities:
    - Fetch current rates for staking, lending, and AMM across protocols
    ...
    - Amnis: Staking (7.5%), Lending (8%), AMM (10%), Blended 8.2%
    - Thala: Staking (7.5%), Lending (8%), AMM (10%), Blended 8.3%
    - Echo: Lending (5.0%)
